  ...
  this.evalWrite = function (evalwrite, result, deviceId) {
    if (evalwrite) { //case we want to write inside a variable
      ...
      evalwrite.forEach(evalW => {
        if (evalW.deviceId) {theDeviceId = evalW.deviceId} //this is specific for listeners and discovery, when one command should be refreshing data of multiple devices (example hue bulbs)
        else {theDeviceId = deviceId}
        //process the value
        let finalValue = self.vault.readVariables(evalW.value, theDeviceId);
        console.log(finalValue)
        finalValue = self.assignTo(RESULT, finalValue, result);
        console.log('about to write :' + finalValue + ' in ' + theDeviceId + '/' + evalW.variable)
        self.vault.writeVariable(evalW.variable, finalValue, theDeviceId); 
      });
    }
  };
    ...
    if (evaldo) { //case we want to trigger a button
      evaldo.forEach(evalD => {
        ...
        console.log(finalDoTest)
        if (finalDoTest) {
          if (evalD.then && evalD.then != '')
          {
            console.log(evalD.then)
